...
#        return image.transform(lambda img: filters.gaussian_filter(img,3.0))
    ...
    def make_results_name(self, name=None):
        return "%s_%s_n=%i_p=%i_a=%i_g=%i_s=%s"%(self.__class__.__name__, name, self.num_particles, self.lp_space, self.weight_power, self.goodness_power, self.STDEVS*self.sample_std_modifier)

    def make_results_dir(self, name=None):
        results_dir = make_path("results", self.make_results_name(name))
    ...
    def export_results(self, export_name):
        for name, draw_all in zip(("only_track", "all_particles"), (False, True)):
            ...
            pngvin_dir = os.path.join(self.make_results_dir(export_name), name + ".pngvin")
