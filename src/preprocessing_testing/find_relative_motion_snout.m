...
edge_response = @(img) sqrt( ...
                            conv2replicate(img,fspecial('prewitt') ).^2+ ...
                            conv2replicate(img,fspecial('prewitt')').^2  ...
                           );
...
image1=gpuArray(double(imread('frames/m8_multi-frames/preprocessed_snout/frame-0536.png')));

start_edge=conv2replicate(edge_response(start),fspecial('gaussian',5*3,5));
start_edge=start_edge/sum(sum(start_edge));

image1_edge=conv2replicate(edge_response(image1),fspecial('gaussian',5*3,5));
