...
\begin{equation}
    ...
    \end{array}
\end{equation}
It can also be identified with $\NN^2\times R^+$ as the tuple $\langle \text{position}, \text{intensity}\rangle$. The \emph{image space} is denoted as $\IS$ in this thesis.

In a computer an \emph{image} is represented as a integer matrix, often 8 bit integers.\footnote{Integers in the range $\interval{0}{255}$.}
