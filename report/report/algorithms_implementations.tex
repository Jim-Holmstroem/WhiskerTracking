...
\begin{comment}
If some state parameters are known to be independent, the database can
be ``compressed'' in the sense that we need fewer observations to fill
the state space. The parameters are then partitioned into independent
\emph{parameter groups}. The parameters in a parameter group may be
internally dependent, but are independent of parameters in other
parameter groups. The extreme cases are when all parameters are
independent, and when all parameters are dependent. In the former
case, each parameter group will contain only one parameter, and in the
latter there will be one single parameter group containing all
parameters. In any case, the database would contain one table for each
parameter group, and the set of stored transition would be the
Cartesian product of these tables.
\begin{example}
  The $x$ positions and velocities of an object may be independent of
  the $y$ positions and velocities. In this case, we would divide the
  parameters into two parameter groups. The database would consist of
  two tables, with transitions for the $x$ and $y$ parts respectively.
\end{example}

The whisker model used treats all parameters as dependent since they
are not known to be independent.
\end{comment}
