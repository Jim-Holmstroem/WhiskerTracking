...
that the chance for point one wants to approximate to fall outside the
sample set is orders of magnitude greater, and the point then needs to
be extrapolated instead of the better alternative of interpolation.
...
the search space in this fashion.
...
\begin{definition}
  ...
  similarity measure for hypothesis response this thesis is defined by
    \begin{equation}
        \label{eq:response}
