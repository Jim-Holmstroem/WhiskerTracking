...
\begin{theorem} %TODO svagare?
    ...
    \begin{equation}
        \argmax{\bar{e}}{\sum\limits_{\Omega}{f(\bar{x})f(\bar{x}-\bar{e})}}=0
    \end{equation}
\end{theorem}
\begin{proof}
    ...
    \begin{equation}
        \begin{split}
            \argmax{e}{&\sum\limits_{\Omega}{W_a^b(x)W_a^b(x-e)}}=\\
            \argmax{e}{&\sum{W_a^b(x)W_{a+e}^{b+e}(x)}}=\\
            \argmax{e}{&\sum{W_{\max(a,a+e)}^{\min(b,b+e)(x)}}=\\
            \argmax{e}{&\Theta(\min(b,b+e)-\max(a,a+e))}=\\
            \left\{&\mathrm{max the min and min the max at the same time}\right\}=\\
            &0
        \end{split}
    \end{equation}
